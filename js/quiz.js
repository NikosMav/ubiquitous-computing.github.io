...
let questions = [], position = 0, score = 0;
const answers = [];
...
function renderQuestion(focus = false) {
  ...
  form.addEventListener('submit', event => {
    ...
    if (checked) {
      position++;
      if (position === questions.length) renderResult(); else renderQuestion(true);
      return;
    }
    ...
    feedback.textContent = isCorrect ? `Σωστά! ${correct}` : `Η σωστή απάντηση είναι: ${correct}. Συνέχισε για να μάθεις περισσότερα.`;
    feedback.hidden = false;
    progress.value = position + 1;
    submit.textContent = position + 1 === questions.length ? 'Δες το αποτέλεσμα' : 'Επόμενη ερώτηση';
    submit.focus();
  });
...
function renderResult() {
  ...
  title.tabIndex = -1;
  result.append(title, element('p', 'Το αποτέλεσμα είναι μια αφετηρία για εξερεύνηση, όχι βαθμός ή πιστοποίηση.'));
  const route = mode === 'intro' ? learningRoute(score, questions.length) : { title: 'Επιστροφή στα κεφάλαια', href: 'reading.html', description: 'Μπορείς να επιστρέψεις στη θεωρία και να δοκιμάσεις ξανά όποτε θέλεις.' };
  result.append(element('p', route.description));
  const actions = element('div', '', 'actions');
  const link = element('a', route.title, 'button'); link.href = route.href;
  const download = element('button', 'Αποθήκευση αποτελέσματος', 'secondary'); download.type = 'button';
  download.addEventListener('click', () => {
    const text = `Διάχυτη υπολογιστική\n${mode === 'intro' ? 'Quiz αφετηρίας' : 'Τελικό quiz'}: ${score}/${questions.length}\n\n` + answers.map(a => `${a.question}\nΗ απάντησή σου: ${a.selected}\nΣωστή απάντηση: ${a.correct}`).join('\n\n') + '\n\nΠτυχιακή εργασία Νικόλαου Μαυραπίδη, ΕΚΠΑ (2023).\nhttps://nikosmav.github.io/ubiquitous-computing.github.io/\n';
    const url = URL.createObjectURL(new Blob(['\ufeff', text], { type: 'text/plain;charset=utf-8' }));
    const a = element('a'); a.href = url; a.download = 'ubiquitous-computing-quiz.txt'; a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  });
  const restart = element('button', 'Δοκίμασε ξανά', 'secondary'); restart.type = 'button';
  restart.addEventListener('click', () => { position = 0; score = 0; answers.length = 0; questions = sampleQuestions(questions, questions.length); renderQuestion(true); });
  actions.append(link, download, restart); result.append(actions); content.append(result); title.focus();
}
...
async function load() {
  ...
  try {
    ...
    const retry = element('button', 'Νέα προσπάθεια'); retry.type = 'button';
    retry.addEventListener('click', () => { content.replaceChildren(); load(); });

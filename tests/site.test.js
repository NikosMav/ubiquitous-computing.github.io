...
test('all page assets, local links and fragment targets resolve', async () => {
  ...
  const html = Object.fromEntries(await Promise.all(pages.map(async file => [file, await readFile(new URL(file, root), 'utf8')])));
  for (const [file, source] of Object.entries(html)) {
    ...
    for (const [, url] of source.matchAll(/(?:href|src)="([^"]+)"/g)) {
      ...
      if (hash && html[resolved]) assert.ok(html[resolved].includes(`id="${hash}"`), `${file}: ${url}`);
    }
    assert.doesNotMatch(source, /@latest|emailjs|webflow\.js|mobile-screen|documents\/blue-squares/);
  }
});

...
const routes = ['index.html', 'reading.html', 'intro_quiz.html', 'knowledge_quiz.html', 'face_recognition.html', 'hand_gestures.html', 'pose_detection.html'];
for (const route of routes) {
  test(`${route}: accessible, no overflow or failed local resources`, async ({ page }) => {
    const errors = [], failures = [];
    page.on('pageerror', error => errors.push(error.message));
    page.on('response', response => { if (response.url().startsWith('http://localhost') && response.status() >= 400) failures.push(response.url()); });
...
for (const [route, bankFile, count] of [['intro_quiz.html', 'intro-questions.json', 8], ['knowledge_quiz.html', 'questions.json', 7]]) {
  test(`${route}: complete, download and restart`, async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const title = await page.locator('legend').innerText();
      const question = bank.find(q => q.question === title);
      await page.getByRole('radio', { name: question.answer ?? question.correctAnswer, exact: true }).check();
      await page.getByRole('button', { name: 'Έλεγχος απάντησης' }).click();
      await expect(page.locator('.feedback')).toContainText('Σωστά!');
      await expect(page.getByRole('radio').first()).toBeDisabled();
      await page.getByRole('button', { name: i + 1 === count ? 'Δες το αποτέλεσμα' : 'Επόμενη ερώτηση' }).click();
    }
    await expect(page.locator('.quiz-result h2')).toHaveText(`${count} / ${count}`);
    if (route.startsWith('intro')) await expect(page.getByRole('link', { name: 'Ξεκίνα με τις εφαρμογές' })).toHaveAttribute('href', 'reading.html#mobile-fp');
...
test('camera denial is recoverable and does not download a model', async ({ page }) => {
  ...
  expect(requests.some(url => url.includes('mediapipe-models') || url.includes('/vendor/'))).toBe(false);
});
...
test('cancel pending permission releases a late-arriving stream', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    window.resolveCamera({ getTracks: () => [{ stop: () => { window.trackStopped = true; } }] });
  });
  ...
  test(`${route}: real worker/model starts, processes frames and stops`, async ({ page }) => {
    await page.addInitScript(() => {
      navigator.mediaDevices.getUserMedia = async () => {
        ...
        const ctx = canvas.getContext('2d'); ctx.fillStyle = '#ddd'; ctx.fillRect(0, 0, 640, 480);
    ...
    await page.goto('/' + route);
    await page.locator('#start-camera').click();
    await expect(page.locator('#camera-status')).toContainText('Το πείραμα λειτουργεί', { timeout: 65000 });
